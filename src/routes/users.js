...
export default async function userRoutes(app) {
    ...
    app.post("/newTask", { onRequest: [authenticate] }, async (request, reply) => {
        ...
        try {
            const task = await prisma.task.create({
                data: {
                    ...
                    deadline: deadline && !isNaN(Date.parse(deadline))
                        ? new Date(deadline)
                        : null,
    ...
    app.put("/tasks/:id", { onRequest: [authenticate] }, async (request, reply) => {
        ...
        const { title, description, deadline, completed } = request.body;
        try {
    ...
    app.patch("/tasks/:id/complete", { onRequest: [authenticate] }, async (request, reply) => {
        const { id } = request.params;
    ...
    app.delete("/tasks/:id", { onRequest: [authenticate] }, async (request, reply) => {
        const { id } = request.params;
        try {
